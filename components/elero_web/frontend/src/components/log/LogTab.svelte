<script>
  ...
  $effect(() => {
    if (filteredLog.length > prevLen && s.logAutoScroll && logBox) {
...
<div class="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
  <div class="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-gray-200 dark:border-gray-700">
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button onclick={startCapture} disabled={s.logCapture}
      ...
      <button onclick={stopCapture} disabled={!s.logCapture}
    ...
    <div class="flex items-center gap-3">
      <label class="flex items-center gap-1.5 text-xs text-gray-600 dark:text-gray-400 cursor-pointer">
        <input type="checkbox" bind:checked={s.logAutoScroll}
  ...
  <div bind:this={logBox}
    ...
    {#if filteredLog.length === 0}
      <div class="text-center py-8 text-gray-500">
        No log entries. {!s.logCapture ? 'Start capture first.' : ''}
      </div>
    {:else}
