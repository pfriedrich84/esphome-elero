<script>
  ...
  import { s, startScan, stopScan, downloadYaml, getDiscoveredNew, getDiscoveredKnown } from '../../lib/stores.svelte.js'
...
<div class="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 mb-4">
  <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
    ...
    <div class="flex items-center gap-2">
      ...
      <span class="text-xs text-gray-500 dark:text-gray-400">{s.scanning ? 'Scanning...' : 'Idle'}</span>
    </div>
  </div>
  <div class="p-4">
    <div class="flex flex-wrap gap-2">
      <button onclick={startScan} disabled={s.scanning}
      ...
      <button onclick={stopScan} disabled={!s.scanning}
...
{#if getDiscoveredNew().length === 0 && !s.scanning}
