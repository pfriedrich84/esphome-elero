<script>
  ...
  import { s, startDump, stopDump, clearDump, downloadDump } from '../../lib/stores.svelte.js'
</script>
...
<div class="bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700">
  <div class="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
    ...
    <div class="flex items-center gap-2">
      <Badge color={s.dumpActive ? 'green' : 'dark'} class="text-xs">{s.dumpActive ? 'Active' : 'Idle'}</Badge>
      {#if s.dumpPackets.length > 0}
        <Badge color="blue" class="text-xs">{s.dumpPackets.length}</Badge>
      {/if}
    </div>
  </div>
  <div class="p-4 space-y-3">
    <div class="flex flex-wrap gap-2">
      <button onclick={startDump} disabled={s.dumpActive}
      ...
      <button onclick={stopDump} disabled={!s.dumpActive}
      ...
      <button onclick={downloadDump} disabled={s.dumpPackets.length === 0}
    ...
    {#if s.dumpPackets.length > 0}
      <div class="overflow-x-auto max-h-[360px] overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <table class="w-full text-xs text-left">
          ...
          <tbody>
            {#each [...s.dumpPackets].reverse() as pkt (pkt.t)}
