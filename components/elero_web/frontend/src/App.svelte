...
<nav class="bg-white border-b border-gray-200 px-4 py-2.5 dark:bg-gray-800 dark:border-gray-700 fixed w-full z-20 top-0 start-0">
  <div class="flex flex-wrap items-center justify-between max-w-screen-xl mx-auto">
    <div class="flex items-center gap-3">
      ...
      <span class="text-lg font-semibold text-gray-900 dark:text-white">{s.deviceName || 'Elero Blind Manager'}</span>
    </div>
    <div class="flex items-center gap-3">
      <span class="text-xs text-gray-500 dark:text-gray-400 hidden sm:inline">{uptimeFmt(s.uptimeMs)}</span>
...
<main class="pt-16 max-w-screen-xl mx-auto px-4 pb-8">
  <div class="border-b border-gray-200 dark:border-gray-700 mt-4">
    <ul class="flex flex-wrap -mb-px text-sm font-medium text-center">
      {#each [
        { id: 'devices', label: 'Devices', count: s.covers.length + s.lights.length, color: 'primary' },
        { id: 'discovery', label: 'Discovery', count: getDiscoveredNew().length, color: 'primary' },
        { id: 'log', label: 'Log', count: s.logEntries.length, color: 'yellow' },
        { id: 'config', label: 'Configuration', count: 0, color: '' },
      ] as t (t.id)}
        <li class="me-2">
          <button onclick={() => s.tab = t.id}
            class="inline-flex items-center gap-2 p-4 border-b-2 rounded-t-lg {s.tab === t.id ? 'text-primary-600 border-primary-600 dark:text-primary-500 dark:border-primary-500' : 'border-transparent hover:text-gray-600 hover:border-gray-300 dark:hover:text-gray-300 text-gray-500 dark:text-gray-400'}">
            {t.label}
            {#if t.count > 0}
              <span class="inline-flex items-center justify-center w-5 h-5 text-xs font-semibold rounded-full
                {t.color === 'yellow' ? 'text-yellow-800 bg-yellow-100 dark:bg-yellow-900 dark:text-yellow-300' : 'text-primary-800 bg-primary-100 dark:bg-primary-900 dark:text-primary-300'}">
                {t.count}
              </span>
            {/if}
          </button>
...
{#if s.toast.show}
  <div class="fixed bottom-5 left-1/2 -translate-x-1/2 z-50">
    <div class="flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg text-sm
      {s.toast.error ? 'text-red-800 bg-red-50 dark:bg-red-800 dark:text-red-200' : 'text-green-800 bg-green-50 dark:bg-green-800 dark:text-green-200'}">
      {s.toast.error ? '&#x2716;' : '&#x2714;'}
      {s.toast.msg}
    </div>
  </div>
